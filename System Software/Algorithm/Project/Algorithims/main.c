...
void scanWalls (void)
{
   ...
   if(sensorOn(readRightWall()) || sensorOn(readRightAlign()))
      addWall((newMaze.mouseHeading + 1) % 4);

   if(sensorOn(readLeftWall()) || sensorOn(readLeftAlign()))
...
void main(void)
{
   ...
   do               // Map maze
   {
     scanWalls();
     solveMaze();
     turnToBestNeighbor( getBestNeighbor() );
     nextCellGo();
     rightTurn();
   }while(newMaze.values[newMaze.mouseRow][newMaze.mouseColumn] != 0);
